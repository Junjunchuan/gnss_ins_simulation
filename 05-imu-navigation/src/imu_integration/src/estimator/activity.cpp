...
void Activity::Init(void) {
    ...
    odom_ground_truth_sub_ptr = std::make_shared<OdomSubscriber>(private_nh_, "/sim/sensor/odom", 1000000);
...
bool Activity::ReadData(void) {
    ...
    if (initialized_) {
        odom_ground_truth_sub_ptr->ParseData(odom_save_buff_);
        
        if (static_cast<size_t>(0) == odom_save_buff_.size())
            return false;  
...
bool Activity::HasData(void) {
    ...
    if (
        initialized_ && 
        static_cast<size_t>(0) == odom_save_buff_.size()
    ) {
        return false;
    }
    return true;
...
bool Activity::UpdatePose(void) {
    if (!initialized_) {
        ...
        current_odom_data_ = odom_data_buff_.back();
        ...
        pose_resolve_.open("/home/chongda517/data/pose.txt",std::fstream::out);
        gt_.open("/home/chongda517/data/gt.txt",std::fstream::out);
    } else {
        //
        // TODO: implement your estimation here
        
        // get deltas:
	   Eigen::Vector3d angular_delta_body;
	   GetAngularDelta(1,0,angular_delta_body);
        // update orientation:
	   Eigen::Matrix3d R_curr, R_prev;
	   UpdateOrientation(angular_delta_body,R_curr, R_prev); 
        // get velocity delta:
	   double delta_t;
       ...
       //std::cout << "current pose: " << pose_ << "current vel: " << vel_ << std::endl;
        imu_data_buff_.pop_front();
        odom_save_buff_.pop_front();
    }
...
bool Activity::GetAngularDelta(
    ...
    current_imu_data_ = imu_data_buff_.at(index_curr);
    if(odom_save_buff_.size() > index_curr){
        current_odom_data_ = odom_save_buff_.at(index_curr);
        Eigen::Quaterniond gt_q(current_odom_data_.pose.block<3, 3>(0, 0));
        gt_ <<  std::fixed << current_imu_data_.time << " " << current_odom_data_.pose(0,3) << " " << current_odom_data_.pose(1,3)
          << " " << current_odom_data_.pose(2,3) << " " << gt_q.coeffs()(0) 
          << " " << gt_q.coeffs()(1) << " " << gt_q.coeffs()(2) << " " << gt_q.coeffs()(3) << std::endl;
    }
...
void Activity::UpdatePosition(const double &delta_t, const Eigen::Vector3d &velocity_delta) {
    ...
    Eigen::Quaterniond pose_q(pose_.block<3, 3>(0, 0));
    pose_resolve_ <<  std::fixed << current_imu_data_.time << " " << pose_(0,3) << " " << pose_(1,3)
          << " " << pose_(2,3) << " " << pose_q.coeffs()(0)
          << " " << pose_q.coeffs()(1) << " " << pose_q.coeffs()(2) << " " << pose_q.coeffs()(3) << std::endl;
