...
def gnss_ins_sim_recorder():
    ...
    # get local g
    earth_param = geoparams.geo_param([32,120,0])
    g = earth_param[2]
    print g
    ...
    topic_name_odom = rospy.get_param('/gnss_ins_sim_recorder_node/topic_name/odom')
    ...
    with rosbag.Bag(
        ...
        for measurement in imu_simulator:
            ...
            odom_msg.child_frame_id = 'inertial'
            odom_msg.header.stamp = timestamp_start + rospy.Duration.from_sec(measurement['stamp'])
            # b. set orientation
            odom_msg.pose.pose.orientation.w = measurement['data']['ref_att_quat_w']
            odom_msg.pose.pose.orientation.x = measurement['data']['ref_att_quat_x']
            odom_msg.pose.pose.orientation.y = measurement['data']['ref_att_quat_y']
            odom_msg.pose.pose.orientation.z = measurement['data']['ref_att_quat_z']
            # c. pos:
            
            if ag == 0:
                init_pose_x = measurement['data']['ref_pos_x']
                init_pose_y = measurement['data']['ref_pos_y']
                init_pose_z = measurement['data']['ref_pos_z']
                odom_msg.pose.pose.position.x = 0.0
                odom_msg.pose.pose.position.y = 0.0
                odom_msg.pose.pose.position.z = 0.0
                ag += 1
            else:
                odom_msg.pose.pose.position.x = measurement['data']['ref_pos_x'] - init_pose_x
                odom_msg.pose.pose.position.y = measurement['data']['ref_pos_y'] - init_pose_y
                odom_msg.pose.pose.position.z = measurement['data']['ref_pos_z'] - init_pose_z
            # d. pos:
